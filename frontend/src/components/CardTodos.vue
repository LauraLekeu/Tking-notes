                ...
                <div  v-if="todo.completed === 1" class="card-style-2 completed" style="margin-bottom: 1em">
                    <div class="card-container">
                        ...
                        <div class="texts-content">
                            <p class="text-title"> 
                               <strong style="font-weight:500"> 
                                 {{ formatDate(todo.deadline) }}</strong> - {{ course(todo).name }}  
                            </p>
                            <p class="text-content"> {{ todo.content }} </p>
                        </div>
                    </div>
                    <div class="card-icons">
                        <svg @click="supprimer(todo.id)" xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor"
 ...
 import axios from 'axios';
    ...
    export default {
        ...
        methods: {
            ...
            supprimer(id) {
                console.log(id)
                axios.post('http://127.0.0.1:8000/api/deleteTodos', {id})
                .then(response => {
